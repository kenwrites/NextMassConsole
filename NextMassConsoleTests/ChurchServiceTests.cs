...
namespace NextMassConsoleTests
{
    public class ChurchServiceTests
    {
        ...
        [Theory]
        [InlineData(84.22322, 23.22344, "Saturday", 9, 30, "Saturday", 11, 30, "Saint Testy McTesterton")]
        [InlineData(24.44932, 25.224343, "Sunday", 7, 00, "Sunday", 9, 00, "Saint Testy McTesterton2")]
        public void AddChurch_Church_Tests(double lattitude, double longitude, string day1, int hour1, int minute1, string day2, int hour2, int minute2, string churchName)
        {
            _churchService.RemoveAllChurchesForTesting();
            var churchLoc = new Location { Latitude = lattitude, Longitude = longitude };
            var massTimes = new List<IMassTime>
            {
                new MassTime { Day = Enum.Parse<DayOfWeek>(day1), Hour = hour1, Minute = minute1 },
                new MassTime { Day = Enum.Parse<DayOfWeek>(day2), Hour = hour2, Minute = minute2 },
            };
            var expected = new Church { Coordinates = churchLoc, MassTimes = massTimes, Name = churchName };
            ...
            Assert.True(
                ...
                actualListAfter.Find(c => c.Name == expected.Name)
                .MassTimes == expected.MassTimes);
        }
        ...
        {
            var churchLoc = new Location { Latitude = 21.85843, Longitude = 32.44373 };
            var massTimes = new List<IMassTime>
            {
                new MassTime { Day = DayOfWeek.Saturday, Hour = 5, Minute = 30 },
                new MassTime { Day = DayOfWeek.Sunday, Hour = 7, Minute = 30 },
            };
            var newChurch = new Church { Coordinates = churchLoc, MassTimes = massTimes, Name = "Test1" };
            _churchService.AddChurch(newChurch);

            churchLoc = new Location { Latitude = 31.33223, Longitude = 62.4432234 };
            massTimes = new List<IMassTime>
            {
                new MassTime { Day = DayOfWeek.Saturday, Hour = 19, Minute = 30 },
                new MassTime { Day = DayOfWeek.Sunday, Hour = 9, Minute = 00 },
            };
            newChurch = new Church { Coordinates = churchLoc, MassTimes = massTimes, Name = "Test2" };
            _churchService.AddChurch(newChurch);

            // identical church 1
            churchLoc = new Location { Latitude = 18.34432, Longitude = 43.223423 };
            massTimes = new List<IMassTime>
            {
                new MassTime { Day = DayOfWeek.Saturday, Hour = 20, Minute = 30 },
                new MassTime { Day = DayOfWeek.Sunday, Hour = 11, Minute = 00 },
            };
